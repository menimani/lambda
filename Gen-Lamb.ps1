...
$lines += "`t`t/**"
$lines += "`t`t * ラムダ式を実行する（引数なし、戻り値なし）"
...
$lines += "`t`t/**"
$lines += "`t`t * ラムダ式を実行する（引数なし、戻り値あり）"
...
for ($i = 1; $i -le $count; $i++) {
    ...
    $lines += "`t`t/**"
    $lines += "`t`t * ラムダ式を実行する（引数${i}個、戻り値なし）"
    for ($j = 1; $j -le $i; $j++) {
    ...
    # ofメソッド
    $lines += "`t/**"
    $lines += "`t * ラムダ式を保持する（引数${i}個、戻り値なし）"
    $lines += "`t * <p>"
    $lines += "`t * 即時実行ではなく、後から {@code da(...)} を呼び出すことで任意のタイミングで実行可能です。"
    $lines += "`t * @param action 保持するラムダ式"
    for ($j = 1; $j -le $i; $j++) {
    ...
    # daメソッド
    $lines += "`t/**"
    $lines += "`t * ラムダ式を即時実行する（引数${i}個、戻り値なし）"
    $lines += "`t * @param action 実行するラムダ式"
    for ($j = 1; $j -le $i; $j++) {
...
for ($i = 1; $i -le $count; $i++) {
    ...
    $lines += "`tpublic interface $interfaceName<" + ($typeParams -join ", ") + "> {"
    $lines += "`t`t/**"
    $lines += "`t`t * ラムダ式を実行する（引数${i}個、戻り値あり）"
    for ($j = 1; $j -le $i; $j++) {
        $lines += "`t`t * @param arg$j 第${j}引数"
    }
    $lines += "`t`t * @return ラムダ式の戻り値"
    ...
    # ofメソッド
    $lines += "`t/**"
    $lines += "`t * ラムダ式を保持する（引数${i}個、戻り値あり）"
    $lines += "`t * <p>"
    $lines += "`t * 即時実行ではなく、後から {@code da(...)} を呼び出すことで任意のタイミングで実行可能です。"
    $lines += "`t * @param action 保持するラムダ式"
    for ($j = 1; $j -le $i; $j++) {
    ...
    # daメソッド
    $lines += "`t/**"
    $lines += "`t * ラムダ式を即時実行する（引数${i}個、戻り値あり）"
    $lines += "`t * @param action 実行するラムダ式"
    for ($j = 1; $j -le $i; $j++) {
        $lines += "`t * @param arg$j 第${j}引数"
    }
